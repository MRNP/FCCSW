...
StatusCode DelphesSaveMet::saveOutput(Delphes& delphes, const fcc::MCParticleCollection& /*mcParticles*/) {
  ...
  logging.open("metData.txt", std::ios_base::app); 
  //int metCount = 0;
  for (int j = 0; j < delphesMETColl->GetEntries(); ++j) {
    ...
    logging << abs(candMET->Momentum.Eta()) << " " << candMET->Momentum.Pt() << " " << candMET->Momentum.Px() << " " << candMET->Momentum.Py() << " " << (-(candMET->Momentum)).Phi() << std::endl;
